  ...
  _createClass(Poloniex, [{
    ...
    value: function initOrderBook(market) {
      ...
      socket.onmessage = function (msg) {
        if (msg && msg.data) {
          _this2.parseOrderDelta(msg.data, market);
        }
      };
    ...
    key: 'parseOrderDelta',
    value: function parseOrderDelta(orderDelta, market) {
      var _this3 = this;

      var data = JSON.parse(orderDelta);
      console.log("What is polo market: ", market);
      ...
      if (data && data[2] && data[2][0] && data[2][0][1] && data[2][0][1].hasOwnProperty('orderBook')) {
        ...
        var bids = bidRates.reduce(function (aggregator, bid) {
          ...
          aggregator[_this3.exchangeName + market + bid] = order;
          return aggregator;
        }, {});
        ...
        var asks = askRates.reduce(function (aggregator, ask) {
          ...
          aggregator[_this3.exchangeName + market + ask] = order;
          return aggregator;
        }, {});
          ...
          if (delta[0] === 'o') {
            if (delta[1]) {
              ...
              var bidChange = {
                type: 'BID_UPDATE',
                market: market,
                rateString: _this3.exchangeName + market + delta[2],
