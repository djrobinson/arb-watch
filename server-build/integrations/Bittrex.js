...
var Bittrex = function (_Exchange) {
  ...
  _createClass(Bittrex, [{
        ...
        if (data.hasOwnProperty('R')) {
          ...
          zlib.inflateRaw(raw, function (err, inflated) {
            if (!err) {
              ...
              boundParser('ORDER_BOOK_INIT', _json, market);
              console.log("Bittrex market: ", market);
    ...
    key: 'parseOrderDelta',
    value: function parseOrderDelta(type, orderDelta, market) {
      var _this3 = this;
      ...
      if (type === 'ORDER_BOOK_INIT' && orderDelta['Z'] && orderDelta['S']) {
        ...
        var bids = sortedBids.reduce(function (aggregator, bid) {
          ...
          aggregator[_this3.exchangeName + market + bid.R.toString()] = order;
          return aggregator;
        }, {});
        var asks = sortedAsks.reduce(function (aggregator, ask) {
          ...
          aggregator[_this3.exchangeName + market + ask.R.toString()] = order;
          return aggregator;
        }, {});
        var initOrderBook = {
          type: type,
      ...
      if (type === 'ORDER_DELTA' && orderDelta['Z'] && orderDelta['S']) {
        orderDelta['Z'].forEach(function (change) {
          var orderDelta = {
            type: 'BID_UPDATE',
            market: market,
            rateString: _this3.exchangeName + market + change.R.toString(),
            rate: change.R,
